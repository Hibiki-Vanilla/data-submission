<template>
  <section v-loading="loading" class="page">
    ...
    <!-- bottom -->
    <div class="bottomRow">
      <el-button v-if="state === 0 && conStatus === 1" @click="saveSubmit()">
        {{ $t("editFill.save") }}
      </el-button>
...
<script>
import { getFormDataEditInit, putFormDataEditPersonal, getListProvince } from "@/api";
...
export default {
  ...
  methods: {
    ...
    returnChange() {
      this.$router.push(`/fillInList/${this.$route.params.topId}`);
    },
    fillAgainChange() {
      this.conStatus = 1;
    },
    async getInfo() {
      ...
      if (res?.success) {
        ...
        for (let key in formStructure.schema.properties) {
          ...
          formStructure.schema.properties[key]["err:required"] = this.$t(
            "editFill.noempty"
          );
          if (
    ...
    handlerSubmit(e) {
      this.formRef.validate(async (valid) => {
        if (valid) {
          ...
          const res = await putFormDataEditPersonal(this.$route.params.id, postObj, 1);
          if (res?.success) {
    ...
    async saveSubmit() {
      ...
      for (let key in list) {
        if (
          list[key].storeName === "multSelect" ||
          (list[key].storeName === "select" && list[key]["ui:widget"] === "RadioWidget")
        ) {
          let arrObj = {};
          if (this.formData[key] && Array.isArray(this.formData[key])) {
            this.formData[key].map((item, index) => {
          ...
          if (list[key]["ui:options"]) {
            list[key]["ui:options"].map((dep, depIndex) => {
              if (dep.value === this.formData[key]) {
                arrObj[`${this.formData[key]}`] = list[key]["ui:options"][depIndex].label;
              }
            });
            postObj[key] = arrObj;
          } else {
      ...
      const res = await putFormDataEditPersonal(this.$route.params.id, postObj, 0);
      if (res?.success) {
        this.$message.success(this.$t("editFill.submitSuccess"));
        this.formRef.resetFields();
      }
